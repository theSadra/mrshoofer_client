...
export function TripProvider({ children }: { children: ReactNode }) {
  ...
  const fetchTripData = async (token: string) => {
    ...
    if (isFetchingRef.current) {
      console.log("InlineTripProvider: Already fetching, skipping duplicate");
      return;
    }

    console.log("InlineTripProvider: Fetching trip data for token:", token);
    ...
    try {
      ...
      console.log("InlineTripProvider: Fetch response status:", response.status);
      ...
      console.log("InlineTripProvider: Fetched trip data:", data);
      setTripData(data);
      setError(null);
    } catch (err) {
      console.error("InlineTripProvider: Error fetching trip:", err);
  ...
  useEffect(() => {
    if (tripToken) {
      console.log("InlineTripProvider: Token changed, fetching data:", tripToken);
      fetchTripData(tripToken);
    } else {
      console.log("InlineTripProvider: No token, clearing data");
      setTripData(null);
      setError(null);
    }
  }, [tripToken]);
  ...
  console.log("InlineTripProvider: Rendering with value:", {
