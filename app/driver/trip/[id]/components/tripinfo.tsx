...
type TripInfoProps = {
  trip: Prisma.TripGetPayload<{
    include: { Location: true; Passenger: true; Driver: true };
  }> | null;
};
...
function TripInfo({ trip }: TripInfoProps) {
  ...
  // Handle case where trip is null or undefined (after hooks)
  if (!trip) {
    return (
      <div className="w-full h-screen min-h-0 bg-white flex flex-col overflow-hidden">
        ...
        <div className="w-full bg-white border-t p-4 flex justify-center">
          <button className="w-full max-w-md bg-blue-600 text-white py-3 rounded-xl shadow-lg text-lg font-bold">
            بازگشت به صفحه اصلی
          </button>
        </div>
      </div>
