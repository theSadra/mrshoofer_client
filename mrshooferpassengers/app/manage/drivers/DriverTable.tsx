...
import { Table, TableHeader, TableColumn, TableBody, TableRow, TableCell, Button, Spinner, Input } from "@heroui/react";
...
export default function DriverTable({ onEdit, hideSearchBar = false }: DriverTableProps) {
        ...
        <div className="w-full">
            ...
            <Table aria-label="جدول رانندگان" className="mb-4 relative">
                ...
                <TableBody>
                    {loading ? (
                        ...
                        drivers.map((driver) => (
                            <TableRow key={driver.id}>
                                <TableCell className="font-semibold flex items-center gap-2">
                                    <Icon icon="solar:user-circle-bold-duotone" width={34} height={34} className="text-zinc-500" />
                                    {driver.Firstname} {driver.Lastname}
                                </TableCell>
                                <TableCell className="text-blue-500">{driver.PhoneNumber}</TableCell>
                                <TableCell className="text-sm ">{driver.CarName}</TableCell>
                                <TableCell>
                                    <Button size="sm" variant="flat" color="primary" onClick={() => onEdit(driver)}>
                                        <Icon icon="solar:pen-new-square-broken" width={18} className="me-1" />ویرایش
                                    </Button>
                                </TableCell>
                            </TableRow>
                        ))
                        ...
                        <TableRow>
                            <TableCell colSpan={4} className="text-center text-default-400 py-8">راننده‌ای یافت نشد.</TableCell>
                        </TableRow>
                    )}
                </TableBody>
            </Table>
            ...
            {hasMore && !loading && (
                <div className="flex justify-center">
                    <Button variant="bordered" onClick={loadMore}>بارگذاری بیشتر</Button>
                </div>
            )}
        </div>
    );
