...
export async function sendDriverSMS(driverphone: string, tripInfo: Trip) {
  ...
  try {
    const result = await smsir.SendVerifyCode(driverphone, templteid, [
...
export async function sendDriverLocationAdded(
  driverphone: string,
  tripInfo: Trip,
) {
  // const link = `${APP_BASE_URL}/trip/info/${tripInfo.SecureToken}`;
...
export async function sendDriverTripCanceled(
  driverphone: string,
  tripInfo: Trip,
) {
  // const link = `${APP_BASE_URL}/trip/info/${tripInfo.SecureToken}`;
